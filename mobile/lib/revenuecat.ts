...
const isPro = (info: CustomerInfo): boolean =>
  Boolean(info.entitlements.active[ENTITLEMENT_PRO]);
...
export function useRevenueCat(ownerId: string | undefined): RcContext {
  ...
  const applyInfo = useCallback(
    (info: CustomerInfo, offerings: PurchasesOfferings | null) => {
      setStatus({
        kind: "ready",
        isPro: isPro(info),
        customerInfo: info,
  ...
  const purchase = useCallback(
    async (pkg: PurchasesPackage) => {
      ...
      applyInfo(
        result.customerInfo,
        status.kind === "ready" ? status.offerings : null,
      );
      return result;
    },
    [applyInfo, status],
...
export function pickPackages(offerings: PurchasesOfferings | null): {
  ...
  const monthly =
    current.monthly ??
    current.availablePackages.find((p) => /month/i.test(p.identifier)) ??
    null;
  const yearly =
    current.annual ??
