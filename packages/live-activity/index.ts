...
const native =
  Platform.OS === 'ios' ? requireOptionalNativeModule<NativeModule>('PushrActivity') : null;
...
export const LiveActivity = {
  ...
  onPushToStartToken(handler: (e: PushToStartTokenEvent) => void): EventSubscription {
    if (!native) return { remove() {} } as EventSubscription;
    return native.addListener('onPushToStartToken', handler);
  },
  onActivityUpdateToken(handler: (e: ActivityUpdateTokenEvent) => void): EventSubscription {
    if (!native) return { remove() {} } as EventSubscription;
