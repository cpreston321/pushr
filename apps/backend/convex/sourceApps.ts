...
import { getSourceAppRole, listAccessibleSourceApps, requireSourceAppRole } from './lib/sharing';
...
export const listMine = query({
  ...
  handler: async (ctx) => {
    ...
    return await Promise.all(sorted.map(({ app, role }) => decorateApp(ctx, app, role)));
  }
});
