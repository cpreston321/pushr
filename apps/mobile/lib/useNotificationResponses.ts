...
export function useNotificationResponses(): void {
  ...
  useEffect(() => {
    ...
    const received = Notifications.addNotificationReceivedListener((n) => {
      const data = n.request.content.data as { liveActivity?: LiveActivityPayload } | undefined;
    ...
    const sub = Notifications.addNotificationResponseReceivedListener((res) => {
      const data = res.notification.request.content.data as
        ...
        | undefined;
      const notificationId = data?.notificationId as Id<'notifications'> | undefined;
      ...
      if (notificationId && (identifier === 'reply' || /^act_[1-4]$/.test(identifier))) {
      ...
      if (identifier === Notifications.DEFAULT_ACTION_IDENTIFIER && notificationId) {
        void markRead({ id: notificationId });
      }
    });
    return () => {
