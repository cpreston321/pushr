...
/**
 * formSheet — sharing UI for a single source app. Pushed on top of the
 * detail sheet via `router.push('/source-app-sharing?id=...')`.
 */
export default function SourceAppSharingScreen() {
...
function Body({ sourceAppId }: { sourceAppId: Id<'sourceApps'> }) {
  ...
  async function sendInvite(email: string, role: 'editor' | 'viewer') {
    try {
      ...
      haptic.error();
      Alert.alert("Couldn't send invite", err?.data?.message ?? err?.message ?? 'Please try again.');
    }
  }
    ...
    showActionSheet({
      ...
      options: [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              await inviteByEmail({ sourceAppId, email: invite.email, role: invite.role });
              haptic.success();
            } catch (err: any) {
              haptic.error();
              Alert.alert(
